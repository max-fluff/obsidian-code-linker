...
describe('root token migration', () => {
  ...
  it('rewrites a hand-written template too', () => {
    const s = migrate({ uriTemplate: 'myeditor://open?f={root}/{path}&l={line}' });
    assert.strictEqual(s.uriTemplate, 'myeditor://open?f={code-root}/{path}&l={line}');
  });
  ...
  it('rewrites the templates of saved viewer presets', () => {
    ...
    assert.strictEqual(s.editors[0].template, 'x://{code-root}/{path}');
  });

  it('leaves a template with no root token alone', () => {
    const s = migrate({ uriTemplate: PRESETS.github });
    assert.strictEqual(s.uriTemplate, PRESETS.github);
  });
  ...
  it('never claims the sibling’s token', () => {
    const s = migrate({ uriTemplate: 'file:///{ref-root}/{path}' });
    assert.strictEqual(s.uriTemplate, 'file:///{ref-root}/{path}');
  });
});
